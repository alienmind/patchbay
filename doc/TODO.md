...
| # | Who | Task | Unblocks |
|---|---|---|---|
| **S3** | you | Four knob checks against files rebuilt today | Whether the strip is done |
...
### S2 has run. What it found

Live 12.4.3.

| # | result |
|---|---|
| S2a | All three load. ARP1's Rate reached nothing (Q24), MFX1 was silent and its two transpose knobs were one idea twice (Q25) |
| S2b | AFX1 loads, Effect sweeps eight chains |
| S2c | Tested against a STALE file: `build/AFXS1.adg` predated the rebinding, so it reported the old low-pass |
| S2d | A send raised by hand in the chain list makes the pad wetter, which is Q23's remaining half working |

**Q24: the arpeggiator ships in ms.** `SyncState` is false out of the box,
which is FREE mode, where `SyncedRate` is not in the path at all and
`FreeRate` is. A switch that selects between two parameters rather than
gating one, and the spec bound the one that was off. `sets("SyncState",
True)` is beside the binding now.

**Q25: a bipolar parameter at macro 0 sits at its extreme.** MidiPitcher's
native range is -128..128, so MFX1 transposed everything down 128 semitones
and made no sound. Now `over=Range(-24, 24)` with `start=63.5`, exactly 0
semitones. `test_a_bipolar_binding_opens_off_centre_or_not_at_all` fails on
the whole class, so it cannot come back on another device.

**Transpose is gone from MFX1.** MidiScale's `Transpose` and MidiPitcher's
`Pitch` are one idea twice; Pitch keeps the slot because it needs no scale
to work. Four slots now: Vel Range, Vel Rand, Pitch, Root.

**And the process defect worth more than any of them.** S2c was run against
a file the edit had not been rebuilt into. `build/` is not regenerated by
editing a spec. `CLAUDE.md` now says to rebuild before asking, because the
answer that comes back otherwise is about the old file and reads exactly
like a fix that did not work.

### S3. The three enum knobs, against files rebuilt today

| # | Do this | Should happen |
|---|---|---|
| S3a | `build/ARP1.adg` on a MIDI track. Is the rate toggle on the metronome rather than ms | Yes. Then Rate steps 1/1 down to 1/32 and Gate shortens notes without silencing them |
| S3b | `build/MFX1.adg`. Play with Pitch centred, then either way | Centred is unison. Left transposes down, right up, 24 semitones each way |
| S3c | `build/AFXS1.adg`, sweep chain, turn Tone | Steps Phaser / Flanger / Doubler |
| S3d | Same file, echo chain, turn Tone | Steps Stereo / Ping Pong / Mid-Side |
